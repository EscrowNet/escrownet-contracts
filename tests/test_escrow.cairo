...
fn test_arbiter_approve() {
    ...
    // approve funds to be released by arbiter
...
#[test]
#[should_panic(expected: 'Unauthorized caller')]
fn test_arbiter_approve_unauthorized_caller() {
    let (contract_address, usdt_contract_address) = __setup__();
    println!("Deployed address: {:?}", contract_address);
...
#[test]
#[should_panic(expected: 'Escrow does not exist')]
fn test_arbiter_approve_non_existent_escrow() {
    let (contract_address, usdt_contract_address) = __setup__();
    println!("Deployed address: {:?}", contract_address);
    ...
    // approve funds to be released by arbiter for non-existent escrow
    start_cheat_caller_address(contract_address, arbiter);
    escrow_contract_dispatcher.arbiter_approve(999); // Non-existent escrow ID
}


#[test]
#[should_panic(expected: 'Escrow is not funded')]
fn test_arbiter_approve_without_funding() {
    let (contract_address, usdt_contract_address) = __setup__();
    println!("Deployed address: {:?}", contract_address);
    ...
    // approve funds to be released by arbiter without funding
    start_cheat_caller_address(contract_address, arbiter);
    escrow_contract_dispatcher.arbiter_approve(escrow_id);
}

#[test]
fn test_arbiter_approve_event_emission() {
    let (contract_address, usdt_contract_address) = __setup__();
    println!("Deployed address: {:?}", contract_address);
    ...
    spy
        .assert_emitted(
            @array![
                (
                    contract_address,
                    EscrowContract::Event::ArbiterApproved(
                        EscrowContract::ArbiterApproved {
                            arbiter: arbiter,
                            escrow_id: escrow_id,
                            time_of_approval: starknet::get_block_timestamp()
                        }
